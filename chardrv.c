...
#define  DEVICE_NAME "chardrv"    ///< The device will appear at /dev/chardrv using this value
#define  CLASS_NAME  "mod"        ///< The device class -- this is a character device driver

MODULE_LICENSE("GPL");            ///< The license type -- this affects available functionality
MODULE_AUTHOR("Declan roche");    ///< The author -- visible when you use modinfo
...
static int __init char_init(void){
   ...
   mutex_init(&char_mutex);  // initialise mutex lock
   return 0;
}
...
static int dev_open(struct inode *inodep, struct file *filep){
   if(!mutex_trylock(&char_mutex)){  // attempt to aquire the mutex lock
...
static ssize_t dev_write(struct file *filep, const char *buffer, size_t len, loff_t *offset){
   ...
   printk(KERN_INFO "Chardrv: Received %d characters from the user\n", len);
   return len;
}
